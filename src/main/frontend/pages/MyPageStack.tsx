...
import { Button, Input, Card, TextArea, UserIcon, CogIcon } from '../components';
    ...
    <div className="space-y-6">
      <Card title="마이페이지">
        <div className="text-center mb-6">
          <img 
            src={currentUser.profilePictureUrl || `https://picsum.photos/seed/${currentUser.id}/128/128`} 
            alt={currentUser.name || 'User Profile'} 
            className="w-32 h-32 rounded-full mx-auto mb-4 border-4 border-primary"
          />
          <h2 className="text-2xl font-semibold text-neutral-800">{currentUser.name}</h2>
          ...
          <Link to="/my-page/profile-edit" className="block">
            <Card className="hover:shadow-xl transition-shadow h-full cursor-pointer">
              <div className="flex items-center space-x-3 p-2">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-neutral-700">프로필 정보 수정</h3>
                  <p className="text-sm text-neutral-500">개인 정보 및 성향을 업데이트하세요.</p>
                </div>
              </div>
            </Card>
          </Link>
          <Link to="/my-page/account-settings" className="block">
            <Card className="hover:shadow-xl transition-shadow h-full cursor-pointer">
              <div className="flex items-center space-x-3 p-2">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-neutral-700">계정 설정</h3>
                  <p className="text-sm text-neutral-500">비밀번호 변경 등 계정 관련 설정을 관리합니다.</p>
                </div>
              </div>
            </Card>
          </Link>
...
export const ProfileEditPage: React.FC = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      setFormData({
        ...
        profilePictureUrl: currentUser.profilePictureUrl || `https://picsum.photos/seed/${currentUser.id}/150/150`,
      });
    } else {
  ...
  const handleAddCustomTag = () => {
    if (customTag && !formData.tags?.includes(customTag.startsWith('#') ? customTag : `#${customTag}`)) {
    ...
    <Card title="프로필 정보 수정">
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex flex-col items-center space-y-4">
            <img 
              src={formData.profilePictureUrl || `https://picsum.photos/seed/${currentUser.id}/150/150`} 
              alt="프로필 미리보기" 
        ...
        <Input label="이름" name="name" value={formData.name || ''} onChange={handleChange} />
        <Input label="나이" name="age" type="number" value={formData.age || ''} onChange={handleChange} />
        <Input label="MBTI" name="mbti" value={formData.mbti || ''} onChange={handleChange} placeholder="예: INTP" />
        
        <div>
          <label className="block text-sm font-medium text-neutral-700 mb-1">성향 태그</label>
          ...
          <div className="flex gap-2">
            <Input 
              ...
              className="flex-grow"
            />
            <Button type="button" onClick={handleAddCustomTag} variant="outline" size="md">추가</Button>
          </div>
        ...
        <TextArea label="자기소개" name="bio" value={formData.bio || ''} onChange={handleChange} rows={4} />
        <Input label="포트폴리오 링크" name="portfolioLink" type="url" value={formData.portfolioLink || ''} onChange={handleChange} placeholder="https://example.com" />
        <TextArea label="좋아하는 협업 스타일" name="preferredStyle" value={formData.preferredStyle || ''} onChange={handleChange} />
        <TextArea label="피하는 협업 스타일" name="avoidedStyle" value={formData.avoidedStyle || ''} onChange={handleChange} />

        <div className="flex justify-end space-x-3 pt-4 border-t">
          <Button type="button" variant="ghost" onClick={() => navigate('/my-page')}>취소</Button>
  ...
  return (
    <Card title="계정 설정">
      ...
      <div className="mt-8 pt-6 border-t border-neutral-200">
        ...
        <p className="text-sm text-neutral-500 mt-1 mb-3">계정을 삭제하면 모든 데이터가 영구적으로 제거됩니다. 이 작업은 되돌릴 수 없습니다.</p>
        <Button variant="danger" onClick={() => alert('계정 삭제 기능 (목업)')}>계정 삭제 요청</Button>
      </div>
    </Card>
  );
};
